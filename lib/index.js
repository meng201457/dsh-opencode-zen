...
/**
 * A stub for any name the gate is asked about but that has no hand-written
 * description below.
 *
 * The measured gate only checks that a tool with the required NAME exists, so a
 * name the relay adds tomorrow must work without a code change — that is the
 * whole point of the editable "required tool names" field. Throwing here would
 * turn a user's edit into a hard failure of every request to the allowlisted
 * host (the middleware does not catch), which is a much worse outcome than a
 * plainly-labelled generic shim.
 * @param name - the required tool name.
 * @returns the description and parameters for its shim.
 */
function genericGateToolStub(name) {
	return Object.freeze({
		description: `Compatibility shim required by the OpenCode Zen free tier, which refuses any tool list without a tool named \`${String(name)}\`. This tool is NOT implemented — never call it; use the real DSH tools instead.`,
...
export function buildGateTool(name, shape) {
	const stub = GATE_TOOL_STUBS[name] ?? genericGateToolStub(name);
...
/**
 * Mount the plugin: install the fetch middleware, observe `llm/stream` for the
 * session id.
 *
 * Export form is load-bearing. `cordis-plugin-loader`'s `unwrapExports()`
 * resolves the plugin it hands to `registry.plugin()` like this:
 *
 * ```js
 * exports = exports.default ?? exports   // a default export REPLACES the namespace
 * if (!exports.__esModule) return exports
 * ```
 *
 * An ESM namespace has no `__esModule`, so the *first* line decides everything:
 * with an `export default`, the plugin becomes that bare value and every other
 * named export is dropped. `registry.plugin()` then records
 * `Config: plugin.Config` as `undefined`, and `@deepseek-ai/dsh-settings`
 * filters an entry with no schema out of `describe()` — the Plugins page shows
 * no form at all, silently. The docs state the rule directly ("export one of
 * these forms; do not mix them"), and no official plugin mixes them. So: named
 * exports only, and never `export default apply`.
 *
...
// Deliberately NO `export default apply` here — see the note on `apply` above.
// A default export makes the Loader resolve this module to the bare function,
// dropping `Config` (no settings form) and `name` (runtime.name stays undefined).
