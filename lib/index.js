...
export const SettingsSchema = z.object({
	...
	injectGateTools: z.boolean().default(true),
	gateToolNames: z.array(z.string()).default([...GATE_TOOL_NAMES]),
...
export function apply(ctx) {
	ctx.inject(["settings"], (settingsCtx) => {
		...
		if (resolved.enabled === true && !clientVersionMeetsFloor(resolved.userAgent)) {
			ctx.logger.warn(`dsh-opencode-zen: user-agent ${JSON.stringify(String(resolved.userAgent))} names no OpenCode version >= ${MIN_CLIENT_VERSION.join(".")}; the Zen free tier will refuse every request (426 UpgradeRequired). Set "OpenCode Zen → User-Agent" to at least opencode/${MIN_CLIENT_VERSION.join(".")}.`);
		}
	});
}
